...
export function createStadium(T,extension=0){
  ...
  const fieldMap=canvasMap(1024,2048,(c,W,H)=>{
    ...
    for(const end of [-1,1]){
      c.fillStyle=end<0?'#16364d':'#111111';c.fillRect(0,z(end<0?-30:25),W,H/12);
      c.save();c.translate(W/2,z(end*27.5));
      if(end>0){c.rotate(Math.PI);drawFomoEyes(c,0,0,H/12*.76,(W/26.667)/(H/60));}
      else{c.fillStyle='#efe9d6';c.font='900 96px Arial';c.textAlign='center';c.textBaseline='middle';c.fillText('GREEK VILLAGE',0,0,W*.92);}
      c.restore();
    }
  ...
  fans.forEach((f,i)=>{
    ...
    color.setHex(hash(i,'fan-shirt')>.27?(f.team?awayBlack:0x2a678a):hash(i,'white-shirt')>.4?0xece3cf:0xd9b35a);fanMesh.setColorAt(i,color);
    phases[i*2]=hash(i,'cheer-phase')*Math.PI*2;phases[i*2+1]=hash(i,'cheer-speed')*2;
    part(seatGeometry,f.x,f.y,f.z,1,1,1,f.team?awayBlack:0x325674,'seat',f.angle);
  });
  ...
  function drawScoreboard(state){
    ...
    c.fillStyle='#235679';c.fillRect(30,86,440,208);c.fillStyle='#111111';c.fillRect(554,86,440,208);
  ...
  function animate(time){
    ...
    for(let i=0;i<22;i++){
      const p=footballPlayer(i,time),stride=p.running?Math.sin(time*10+i)*.55:0,jersey=p.team?awayBlack:0x164d75;
      const pieces=[
      ...
      dummy.position.set(p.x,.505+1.56*.76,p.z);dummy.rotation.set(0,angle,0);dummy.scale.set(.23*.76,.25*.76,.24*.76);dummy.updateMatrix();helmets.setMatrixAt(i,dummy.matrix);helmets.setColorAt(i,color.setHex(p.team?awayBlack:0xe4e8e4));
    }
