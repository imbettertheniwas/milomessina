...
const delay=(member,from)=>Math.min((member-from-1)*.18,3);
...
export function createLiveArrivals(){
  ...
  function start(chapters,time,reduced=false){
  ...
  function pose(member,state,time){
    ...
    const height=falling?ARRIVAL_HEIGHT*(1-fall*fall):settle<1?.32*Math.sin(Math.PI*settle):0;
    return {...state,x:landing.x+(state.x-landing.x)*returning,z:landing.z+(state.z-landing.z)*returning,
      ...
      arrival:{arms:falling?1:1-smooth(settle),crouch:falling?0:Math.sin(Math.PI*Math.min(1,settle))*.18}};
  }
  return {enqueue,start,advance,pose,has:chapter=>active.has(chapter)};
}
