...
export function villageQuality(mobile=typeof matchMedia==='function'&&(matchMedia('(pointer: coarse)').matches||matchMedia('(max-width: 700px)').matches)){
  return mobile?{mobile:true,pixelRatio:2,maxPixelRatio:2,minPixelRatio:1.5,frameRate:30,antialias:true,shadowSize:1024,bannerResolution:1024,terrainResolution:2048}:{mobile:false,pixelRatio:2,maxPixelRatio:2,minPixelRatio:1.25,frameRate:60,antialias:true,shadowSize:2048,bannerResolution:2048,terrainResolution:2048};
}

// Start sharp on Retina phones; preserve a readable floor under GPU pressure.
// Recover detail only after sustained spare frame time.
