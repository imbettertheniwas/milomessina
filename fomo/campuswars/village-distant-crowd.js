...
export function createDistantCrowd(T,capacity){
  ...
  material.onBeforeCompile=shader=>{
    ...
    shader.vertexShader=shader.vertexShader.replace('#include <common>','#include <common>\nattribute float region; attribute vec2 swing; attribute vec3 skin; attribute vec3 pants; attribute vec3 hair; attribute vec2 motion; uniform float crowdTime;');
    shader.vertexShader=shader.vertexShader.replace('#include <color_vertex>','#include <color_vertex>\nif(region>.5&&region<1.5)vColor=skin; else if(region>1.5&&region<2.5)vColor=pants; else if(region>2.5&&region<3.5)vColor=hair; else if(region>3.5)vColor=vec3(.07);');
  ...
  function add(member,state){
    ...
    color.set(member.action==='build'?0xe5a13f:palettes.shirts[member.shirt]);mesh.setColorAt(count,color);
    for(const [name,value] of [['skin',palettes.skin[member.skin]],['pants',palettes.pants[member.pants]],['hair',member.cap?palettes.shirts[member.shirt]:palettes.hair[member.hair]]]){color.set(value);geometry.attributes[name].setXYZ(count,color.r,color.g,color.b);}
  ...
  function finish(){mesh.count=count;mesh.instanceMatrix.needsUpdate=true;if(colorsChanged){mesh.instanceColor.needsUpdate=true;for(const name of ['skin','pants','hair'])geometry.attributes[name].needsUpdate=true;}geometry.attributes.motion.needsUpdate=true;}
