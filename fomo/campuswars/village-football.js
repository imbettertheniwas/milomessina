...
function formation(index){
  ...
  return offense?(i===0?[0,-12]:i<6?[(i-3)*1.05,-9]:[(i-8)*3.1,-10]):i<5?[(i-2)*1.15,-7]:[(i-7.5)*2.6,-2];
}
function route(index,t){
  const offense=index<11,i=index%11,start=formation(index);
  let end;
  ...
  if(t>24){const reset=ease((t-24)/8),next=formation((index+11)%22);x=mix(end[0],-next[0],reset);z=mix(end[1],-next[1],reset);}
  return {x,z};
}
export function footballPlayer(index,time){
  const state=footballState(time),team=index<11?0:1,role=(index+state.play%2*11)%22;
  const p=route(role,state.t),next=route(role,Math.min(31.999,state.t+.025));
  // Team identity stays fixed; each player changes roles on alternate drives.
  ...
  return {x:p.x*turn,z:p.z*turn,angle:Math.atan2(next.x-p.x,next.z-p.z)+(turn<0?Math.PI:0),running:state.t>4&&state.t<19||state.t>24,celebrating:role<11&&state.phase==='TOUCHDOWN',team};
}
export function footballBall(time){
  const state=footballState(time),offense=state.play%2*11,qb=footballPlayer(offense,time),receiver=footballPlayer(offense+8,time);
  ...
  if(state.t<11){const a=(state.t-8)/3,release=footballPlayer(offense,state.play*PLAY_SECONDS+8),catcher=footballPlayer(offense+8,state.play*PLAY_SECONDS+11);return {x:mix(release.x,catcher.x,a),y:1.1+Math.sin(a*Math.PI)*6,z:mix(release.z,catcher.z,a)};}
  if(state.t<24)return {x:receiver.x,y:1.1,z:receiver.z};
  const a=ease((state.t-24)/8),start=footballPlayer(offense+8,state.play*PLAY_SECONDS+24);
