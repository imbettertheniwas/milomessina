...
function revealInvite(out) {
  ...
  const text=invite || `${location.origin}/internal/beta#access=${encodeURIComponent(out.code)}`;
  $('bt-code').hidden=false;
  $('bt-code').innerHTML=`<div class="bt-card-head"><b>${invite?'Your batch invite is ready':'Personal return link replaced'}</b><button class="mini ghost" type="button" data-dismiss-code>Dismiss</button></div><p>${invite?'Anyone with this link can fill in their details and join this beta batch. Save it now; it is only shown once.':'Share this personal link directly with this intern. It opens their profile without a password. Their previous link and sessions are closed.'}</p><div class="bt-code-row"><code>${esc(text)}</code><button class="btn btn-g" data-copy-code type="button">Copy ${invite?'invite link':'personal link'}</button></div>`;
...
$('bt-root').addEventListener('click',async event=>{
  ...
  if(b.hasAttribute('data-invite') || b.hasAttribute('data-code')){
    ...
    b.outerHTML=`<div class="bt-rotate-confirm"><p>${invite?'Replace the shared invite? The previous link will stop accepting new people. Existing members keep access.':'Replace this intern’s personal link and sign them out?'}</p><button class="mini" type="button" ${invite?'data-confirm-invite':'data-confirm-code'}>Replace</button><button class="mini ghost" type="button" data-cancel>Cancel</button></div>`;
  }
