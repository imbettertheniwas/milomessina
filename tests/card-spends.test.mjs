...
test('Arya’s is the only card anybody may borrow', () => {
  ...
  assert.equal(h.call(m, 'subadd', spend('Jesse')).ok, false);
  assert.equal(h.call(m, 'add', spend('Milo')).ok, true);
});
...
test('whoever logged a card line can still fix it, though it is settled', () => {
  ...
  assert.equal(fixed.loggedBy, 'Milo');
...
test('a line moved onto the card is settled by the move, and owed again when moved off', () => {
  const h = harness(), m = h.login('Milo');
  const r = h.call(m, 'add', spend('Milo')).rows[0];
  ...
  const back = h.call(m, 'edit', {id: r.id, ...spend('Milo')}).rows[0];
...
test('a monthly rule can sit on the card, and its lines come out settled', () => {
  ...
  assert.equal(rule.loggedBy, 'Milo');
