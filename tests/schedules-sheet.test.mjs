...
const block = (over = {}) => ({who:'Jesse', label:'CS 106', kind:'class',
  days:[1,3,5], start:'09:00', end:'10:15', ...over});
const add = (h, token, over = {}) => h.call(token, 'add', block(over), 'schedules');
const list = h => h.call(h.login('Jesse'), 'list', {}, 'schedules').schedules;
...
test('an intern owns their own week and nobody else’s; administrators manage all of them', () => {
...
test('"nothing fixed" and a real block cannot both be true at once', () => {
  const h = harness(), m = h.login('Jesse');
  add(h, m);
  assert.equal(h.call(m, 'add', {who:'Jesse', kind:'none'}, 'schedules').ok, true);
  let now = mine(h, 'Jesse');
...
test('a calendar imported twice is one calendar, and hand-typed blocks survive it', () => {
  ...
  assert.equal(h.call(m, 'import', {who:'Jesse', source:'ics', blocks:term}, 'schedules').ok, true);
  assert.equal(mine(h, 'Jesse').length, 3);

  assert.equal(h.call(m, 'import', {who:'Jesse', source:'ics',
...
test('an import keeps the lines it can read and drops the ones it cannot', () => {
  const h = harness(), m = h.login('Jesse');
  const out = h.call(m, 'import', {who:'Jesse', source:'pasted', blocks:[
  ...
  assert.equal(h.call(m, 'import', {who:'Jesse', source:'ics', blocks:[
...
test('clearing a week takes only that week off', () => {
  ...
  assert.equal(h.call(m, 'clear', {who:'Jesse'}, 'schedules').ok, true);
  assert.equal(mine(h, 'Jesse').length, 0);
...
test('a break with no name or no date is not a break', () => {
  const h = harness(), m = h.login('Jesse');
  assert.equal(h.call(m, 'add', {who:'Jesse', kind:'break', label:'Reading week'},
    'schedules').error, 'a break needs the day it starts');
  assert.equal(h.call(m, 'add', {who:'Jesse', kind:'break', from:'2099-03-16'},
    'schedules').error, 'a break needs a name');
  assert.equal(h.call(m, 'add', {who:'Jesse', kind:'break', label:'Backwards',
    from:'2099-03-20', to:'2099-03-16'}, 'schedules').ok, true);
  const [b] = mine(h, 'Jesse');
...
test('a break is edited by its dates, and cannot be turned into a class', () => {
  const h = harness(), m = h.login('Jesse');
  h.call(m, 'add', {who:'Jesse', kind:'break', label:'Spring Break',
...
test('a tab written before breaks existed gains the columns and keeps its rows', () => {
  ...
  const old = mine(h, 'Jesse')[0];
  assert.equal(old.label, 'CS 106');
  assert.equal(old.from, '');
  assert.equal(h.call(m, 'add', {who:'Jesse', kind:'break', label:'Spring Break',
  ...
  assert.equal(mine(h, 'Jesse').length, 2);
});
...
test('a rotation nobody could keep is read as every week rather than refused', () => {
  ...
  [['0/2'], ['3/2'], ['1/9'], ['every other'], ['']].forEach(([week]) => {
    h.call(m, 'clear', {who:'Jesse'}, 'schedules');
    add(h, m, {week});
    assert.equal(mine(h, 'Jesse')[0].week, 'every', JSON.stringify(week) + ' is not a rotation');
  });
});
...
test('an edit can put a block on a rotation and take it off again', () => {
  ...
  assert.equal(mine(h, 'Jesse')[0].week, '1/2');
  h.call(m, 'edit', {id:was.id, label:'CS 106', kind:'class', days:[1,3,5],
    start:'09:00', end:'10:15', week:'every'}, 'schedules');
  assert.equal(mine(h, 'Jesse')[0].week, 'every');
});
...
test('a tab written before rotations existed reads every row as every week', () => {
  ...
  assert.deepEqual(sh.rows[0],
    ['id','updated','who','label','kind','days','start','end','source','from','to','week']);
  assert.equal(mine(h, 'Jesse').filter(s => s.label === 'Lab')[0].week, '1/2');
});
